...
def bText = "kot: 120,00 PLN pies: 150,50 PLN chomik: 10,50 PLN"
bText = bText.replaceAll(/(\d+\.?,?\d+) PLN/) { all, price ->
    String.format("%.2f", price.replace(",", ".").toBigDecimal() / plnToEuroRatio) + ' EUR'
}
println "b)\n" + bText